...
export default async ({
  ...
  await runStep({ title: 'Creating ship.config.mjs' }, async () => {
    ...
    if (dryRun) {
      print(`ship.config.mjs`);
      print(serialize(config));
    } else if (!isConfigEmpty) {
      const filePath = path.resolve(dir, 'ship.config.mjs');
      fs.writeFileSync(
        filePath,
    ...
    return () => {
      if (isConfigEmpty) {
        print(`${info('✔')} No \`ship.config.mjs\` required.`);
      } else {
        print(`${info('✔')} Created \`ship.config.mjs\`.`);
      }
